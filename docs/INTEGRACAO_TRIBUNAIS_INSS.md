...
**Addendum 2026-09-18 — case stage classifier (`classificarSituacao`, `services/datajud/normalizar.ts`).** Measured on the firm's 1,121 CNJ-numbered cases (983 found in Datajud): 712 end in Baixa Definitiva (22) or Definitivo (246); 76 more have a baixa followed only by clerical movements (petição, documento, mudança de assunto…); none has desarquivamento (861); RPV/precatório never appear as complementos in TRF4/TJRS, so "paid" is read from the class (Cumprimento de Sentença…) plus baixa; extinção da execução is code 196. Stages: `ARQUIVADO` (22/246 last, or followed only by the `RUIDO_POS_BAIXA` set), `REVISAR` (substantive movement after a baixa — 24 cases, a person decides), `SUSPENSO` (245/12065 last), `EM_CUMPRIMENTO` (cumprimento/execução class, no baixa), `TRANSITADO` (848, no baixa), `SENTENCIADO` (219/220/221, no 848), `ATIVO`. The sync fills `situacao`/`dtArquivado` only when empty and returns `estagio`; `POST /datajud/search` returns `estagio`, `resultado` and `motivoSituacao`. **Phase 2 should persist `estagio` + `motivo` per case (new columns) and surface the `REVISAR` bucket**; staleness heuristics add nothing (742 cases without movement in 12 months, 712 of them already closed).
